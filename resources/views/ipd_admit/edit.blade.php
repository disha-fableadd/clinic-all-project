...
@section('content')
    <div class="page-wrapper">
        <div class="content">
            ...
            <div class="row">
                <div class="col-12">
                    <form class="form-container" id="ipdform" action="" >
                        @csrf
                        <!-- Step 1 -->
                        <div class="form-step discharge-form1" id="step-1">
                            ...
                            <div id="editipfsuccessMessage" class="alert alert-success" style="display:none;"></div>
                            <div id="editipderrorMessage" class="alert alert-danger" style="display:none;"></div>
...
let branchId = localStorage.getItem("selectedBranchId"); // ✅ branch from localStorage
            ...
            if (ipdId) {
                ...
                $.ajax({
                    ...
                    success: function(ipd) {
                        $('#patientDropdown').val(ipd.patient_id).trigger('change');
                        $('select[name="treatment_id"]').val(ipd.treatment_id).trigger('change');
                        $('select[name="doctor_id"]').val(ipd.doctor_id).trigger('change');

                        $('input[name="admission_date"]').val(ipd.admission_date);
                        $('input[name="room_number"]').val(ipd.room_number);
                        $('input[name="bed_number"]').val(ipd.bed_number);
                    },
                    error: function(xhr) {
                        console.error("Failed to fetch IPD data for editing:", xhr.responseText);
                    }
                });
            }
            ...
            form.on('submit', function(e) {
                ...
                if (form.valid()) {
                    ...
                    $.ajax({
                        url: "/api/ipd-admissions/" + ipdId, // Update
                        type: "POST",
                        data: formData,
                        contentType: false,
                        processData: false,
                        ...
                        success: function(response) {
                            $('#editipdsuccessMessage').text(response.message ||
                                'Saved successfully').show();
                            $('#ipdform')[0].reset();
                            setTimeout(function() {
                        ...
                        error: function(xhr) {
                            ...
                            $('#editipderrorMessage').html(errorMessage).show();
                        }
                    });
                }
            });
